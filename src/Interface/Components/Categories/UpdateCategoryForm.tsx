...
function UpdateCategoryForm() {
	...
	useEffect(() => {
		if (categoryData) {
			...
			setGroupId(categoryData.groupId || null);
		}
	}, [categoryData]);
	...
	const handleSubmit = useCallback(async (e: React.FormEvent) => {
		...
		const updatedCategoryData: ProductCategory = {
			id: parseInt(id),
			name: name,
			...
			<div className="max-w-4xl mx-auto mt-8">
				<div className="bg-red-50 border border-red-200 text-red-700 px-4 py-3 rounded-lg">
					<div className="flex items-center">
						...
						Category data not found. Please go back and try again.
					</div>
				</div>
			</div>
		);
	...
	return (
		<div className="max-w-4xl mx-auto bg-white rounded-lg shadow-sm overflow-hidden">
			<div className="px-6 py-4 border-b border-gray-200 bg-slate-600 text-white">
				...
				<p className="text-sm text-gray-300 mt-1">Modify the category details below</p>
			</div>
